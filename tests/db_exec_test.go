...
func TestDBExec(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			in:       "INSERT INTO companies(id, name) VALUES('id_XXXX', 'hoge')",
			affected: 1,
		},
		{
			name:     "Updates one data",
			in:       "UPDATE companies SET name='fuga' WHERE id='id_0000'",
			affected: 1,
		},
	}
...
func TestDBExecContext(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := db.ExecContext(context.Background(), tt.in)
			if err != nil {
				t.Error(err)
			}
			got, err := res.RowsAffected()
